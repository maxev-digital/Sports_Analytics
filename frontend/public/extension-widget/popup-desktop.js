...
async function loadOpportunities(forceRefresh = false) {
  console.log('[DESKTOP WIDGET] Loading opportunities...', forceRefresh ? '(force refresh)' : '');
  try {
    // Fetch directly from API for desktop app
    const API_BASE = 'https://max-ev-sports.com/api';

    // Fetch all alerts from single endpoint
    ...
    console.log('[DESKTOP WIDGET] Received data:', data);
    console.log('[DESKTOP WIDGET] Data keys:', Object.keys(data));

    // Parse the alerts data structure - API returns objects with .alerts arrays
    ...
    console.log('[DESKTOP WIDGET] arbitrage object:', data.arbitrage);
    console.log('[DESKTOP WIDGET] steam_moves object:', data.steam_moves);
    console.log('[DESKTOP WIDGET] middles object:', data.middles);

    console.log('[DESKTOP WIDGET] Opportunities count:', opportunities.length);
    console.log('[DESKTOP WIDGET] Steam moves count:', steamMoves.length);
    console.log('[DESKTOP WIDGET] Middles count:', middles.length);
    console.log('[DESKTOP WIDGET] Goalie pulls count:', goaliePulls.length);
    console.log('[DESKTOP WIDGET] Quarter reversals count:', quarterReversals.length);
    console.log('[DESKTOP WIDGET] Injury props count:', injuryProps.length);

    if (opportunities.length > 0) {
      console.log('[DESKTOP WIDGET] First opportunity:', opportunities[0]);
    }
    ...
    console.error('[DESKTOP WIDGET] Error loading opportunities:', error);
...
function renderAlertFeed() {
  console.log('[POPUP] renderAlertFeed() called');
  ...
  console.log('[POPUP] Total alerts in feed:', allAlerts.length);

  // Sort by urgency (highest first) - no filtering needed
...
function updateConnectionStatus(connected) {
  ...
  statusDot.className = 'status-dot'; // Clear classes
  if (connected) {
...
function checkConnectionStatus() {
  // For desktop, we always show as connected when widget is open
  updateConnectionStatus(true);
}

function openOpportunity(opportunity) {
  // Open directly in system browser (Electron can use window.open or require('electron').shell)
  console.log('[DESKTOP WIDGET] Opening opportunity:', opportunity);
  // For now, just log - will need Electron shell integration for real browser opening
}

// No background script listener needed for desktop
// Desktop fetches directly from API

// Refresh opportunities every 10 seconds while widget is open
setInterval(loadOpportunities, 10000);

// Sound control functions (Desktop version - simplified)
...
function loadSoundSettings() {
  ...
  updateVoiceButton(voiceEnabled);
}
...
function toggleSound() {
  ...
  updateSoundButton(soundEnabled);
  console.log(`[DESKTOP WIDGET] Sound ${soundEnabled ? 'enabled' : 'disabled'}`);
}
...
function testSound() {
  // Play a simple beep for desktop
  console.log('[DESKTOP WIDGET] Test sound - beep!');
...
// Voice control functions (Desktop version)
function toggleVoice() {
  voiceEnabled = !voiceEnabled;
  localStorage.setItem('voiceEnabled', voiceEnabled);
  updateVoiceButton(voiceEnabled);
  console.log(`[DESKTOP WIDGET] Voice ${voiceEnabled ? 'enabled' : 'disabled'}`);
}
...
function testVoice() {
  ...
  console.log('[DESKTOP WIDGET] Test voice announcement');
  const utterance = new SpeechSynthesisUtterance('Test alert: Arbitrage opportunity detected with $3.50 profit');
