...
export const QuarterReversalAlerts: React.FC = () => {
  ...
  const { opportunities: wsOpportunities, connected: wsConnected, error: wsError } = useQuarterReversalWebSocket('default');
    ...
    return (
      <div className="bg-slate-800 border-4 border-slate-700 rounded-lg p-12 text-center">
        <div className="flex items-center justify-center gap-3 mb-3">
          ...
          <div className="text-slate-400 text-lg">No quarter reversals detected</div>
        </div>
        <div className="text-slate-500 text-sm">Monitoring live NBA games via WebSocket (3s refresh)...</div>
        <div className="text-slate-600 text-xs mt-4">
          Opportunities appear when teams win 2 consecutive quarters (Q1-Q2, Q2-Q3, Q3-Q4)
        </div>
      </div>
    );
  }
