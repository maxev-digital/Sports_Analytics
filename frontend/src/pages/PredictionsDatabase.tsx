...
interface Prediction {
  ...
  result: string | null;
  profit_loss: number;
  market_odds: number | null;  // FanDuel odds (American format)
  odds_source: string | null;  // 'fanduel', 'draftkings', etc.
}
...
// Helper function to format P/L as dollars ($100 flat units)
...
export default function PredictionsDatabase() {
  ...
  const fetchPredictions = async () => {
    try {
      setLoading(true);
      let url = `${getApiUrl('ui/historical-predictions')}?days=${days}&limit=${limit}`;
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-800 border-b-2 border-slate-700">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-center text-slate-300 font-semibold">Odds</th>
                    <th
