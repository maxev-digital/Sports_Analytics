...
def predict_for_sport(sport, games, models, config, teams_cache, bet_type):
    """Generate predictions for a specific sport and bet type using ALL models"""
    predictions = []
    feature_engineer = config['feature_engineer']

    sport_games = [g for g in games if g.get('sport', '').upper() == sport.upper()]
    logger.info(f"
{sport} {bet_type}: Processing {len(sport_games)} games")
    ...
    for game in sport_games:
        try:
            ...
            if bet_type == 'totals':
                market_value = game.get('odds', [{}])[0].get('total') if game.get('odds') else None
            elif bet_type == 'spreads':
                market_value = game.get('odds', [{}])[0].get('home_spread') if game.get('odds') else None
            elif bet_type == 'moneyline':
                market_value = game.get('odds', [{}])[0].get('home_ml') if game.get('odds') else None
            
            if not market_value:
                continue
            
            commence_time_utc = game_state.get('commence_time', '') or game.get('commence_time', '')
            ...
            for model_name, model in models.items():
                try:
                    ...
                    if bet_type == 'totals':
                        edge = ((pred - market_value) / market_value) * 100
                        recommendation = "OVER" if edge > 2.5 else "UNDER" if edge < -2.5 else "NO BET"
                        confidence = "HIGH" if abs(edge) > 5 else "MEDIUM" if abs(edge) > 2.5 else "LOW"
                    elif bet_type == 'spreads':
                        edge = pred - market_value
                        recommendation = "HOME" if edge > 2 else "AWAY" if edge < -2 else "NO BET"
                        confidence = "HIGH" if abs(edge) > 4 else "MEDIUM" if abs(edge) > 2 else "LOW"
                    elif bet_type == 'moneyline':
                        edge = abs(pred - 0.5) * 100
                        recommendation = "HOME" if pred > 0.55 else "AWAY" if pred < 0.45 else "NO BET"
                        confidence = "HIGH" if abs(pred - 0.5) > 0.2 else "MEDIUM" if abs(pred - 0.5) > 0.1 else "LOW"
                    
                    pred_id = f"{sport.lower()}_{bet_type}_{game_date}_{home_team.replace(' ', '_')}_{model_name}"
                    ...
                    predictions.append({
                        ...
                        'edge': round(edge, 2) if bet_type != 'moneyline' else round(edge, 2),
                        'recommendation': recommendation,
                        'confidence': confidence,
                        'model': model_name
                    })

                    logger.info(f"  ✓ {away_team} @ {home_team}: {model_name} {bet_type} = {pred:.2f}")
