"""
Random Forest Model for NHL Totals Prediction
Predicts total goals scored in NHL games

Per sports-betting-models-guide.md:
- NHL is highly random sport with small edges
- Primary approach: Regression with goalie/shot quality adjustment
- Alternative: Expected Goals (xG) based models
- Random Forest captures non-linear goalie performance and matchup factors
"""
...
class RandomForestTotalsModel:
    """Random Forest model for predicting NHL game totals"""
    ...
    def __init__(self):
        ...
        self.model = joblib.load(model_dir / "nhl_random_forest_totals_latest.joblib")
    ...
    def predict(self, game_data: Dict, market_total: Optional[float] = None) -> Dict:
        ...
        if hasattr(self.model, 'estimators_'):
            ...
            # NHL has more variance - normalize differently than NBA
            confidence = 1 / (1 + std_dev / 0.5)  # 0.5 goal std dev baseline
        else:
            std_dev = 0.6  # Default ~0.6 goals
            confidence = 0.70
...
def get_nhl_random_forest_totals_model():
    """Get or create singleton Random Forest totals model instance"""
