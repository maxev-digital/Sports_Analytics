"""
Random Forest Model for NHL Spreads (Puck Line) Prediction
Predicts home_margin (home_goals - away_goals)

Per sports-betting-models-guide.md:
- NHL puck lines typically ±1.5 goals
- Account for empty net variance (trailing teams pull goalie)
- Highly random sport requires conservative edges
- Random Forest captures matchup-specific factors
"""
...
class RandomForestSpreadsModel:
    """Random Forest model for predicting NHL spreads (puck line)"""
    ...
    def __init__(self):
        ...
        self.model = joblib.load(model_dir / "nhl_random_forest_spreads_latest.joblib")
    ...
    def _extract_features(self, game_data: Dict) -> np.ndarray:
        ...
        # Home ice advantage (implicit in training data)
    ...
    def predict(self, game_data: Dict, market_spread: Optional[float] = None) -> Dict:
        ...
        # Get prediction from Random Forest (home margin in goals)
        ...
        if hasattr(self.model, 'estimators_'):
            ...
            confidence = 1 / (1 + std_dev / 1.2)  # 1.2 goal std dev baseline for margins
        else:
            std_dev = 1.4  # Default ~1.4 goals
            confidence = 0.68
...
def get_nhl_random_forest_spreads_model():
