"""
XGBoost Model for NHL Moneyline Prediction
Predicts win probability for home and away teams

Per sports-betting-models-guide.md:
- XGBoost excels at classification with many features
- Handles missing data well (injuries, lineup changes)
...
class XGBoostMoneylineModel:
    """XGBoost classifier for predicting NHL moneyline outcomes"""

    def __init__(self):
        """Initialize XGBoost moneyline model by loading trained model"""
        # Load trained model and metadata
        model_dir = Path(__file__).parent.parent.parent / "ml" / "models"

        self.model = joblib.load(model_dir / "nhl_xgboost_moneyline_latest.joblib")
    ...
    def predict(self, game_data: Dict, market_odds: Optional[Dict] = None) -> Dict:
        ...
        # XGBoost typically has high confidence
        confidence = 0.74
...
def get_nhl_xgboost_moneyline_model():
    """Get or create singleton XGBoost moneyline model instance"""
