"""
LightGBM Model for NHL Totals Prediction
...
- Drop-in replacement for XGBoost with same syntax
"""
...
class LightGBMTotalsModel:
    """LightGBM model for predicting NHL game totals"""

    def __init__(self):
        """Initialize LightGBM totals model by loading trained model"""
        # Load trained model and metadata
        model_dir = Path(__file__).parent.parent.parent / "ml" / "models"

        self.model = joblib.load(model_dir / "nhl_lightgbm_totals_latest.joblib")
    ...
    def predict(self, game_data: Dict, market_total: Optional[float] = None) -> Dict:
        ...
        # LightGBM confidence (typically slightly better than XGBoost)
        std_dev = 0.52  # LightGBM often has slightly lower variance
        confidence = 0.75
...
def get_nhl_lightgbm_totals_model():
    """Get or create singleton LightGBM totals model instance"""
