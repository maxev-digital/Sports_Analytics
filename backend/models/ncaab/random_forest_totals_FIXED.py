"""
Random Forest Model for NCAAB Totals Prediction
...
class NCAABRandomForestModel:
    """Random Forest model for predicting NCAAB totals"""

    def __init__(self):
        """Initialize Random Forest totals model by loading trained model"""
        model_dir = Path(__file__).parent.parent.parent / "ml" / "models"

        self.model = joblib.load(model_dir / "ncaab_random_forest_totals_latest.joblib")
    ...
    def _extract_features(self, game_data: Dict) -> np.ndarray:
        ...
        # Conference and ranking info (with defaults)
        ...
        features[0, 2] = (home_tempo + away_tempo) / 2  # combined_tempo
        features[0, 3] = abs(home_tempo - away_tempo)  # tempo_diff

        # Offensive efficiency features (4-7)
        features[0, 4] = home_off
        features[0, 5] = away_off
        features[0, 6] = (home_off + away_off) / 2  # avg_offense
        features[0, 7] = abs(home_off - away_off)  # off_diff

        # Defensive efficiency features (8-11)
        features[0, 8] = home_def
        features[0, 9] = away_def
        features[0, 10] = (home_def + away_def) / 2  # avg_defense
        features[0, 11] = abs(home_def - away_def)  # def_diff

        # Efficiency margin features (12-14)
        features[0, 12] = home_em
        features[0, 13] = away_em
        features[0, 14] = abs(home_em - away_em)  # em_diff

        # Pace indicators (15-16)
        features[0, 15] = 1.0 if features[0, 2] > 72.0 else 0.0  # is_fast_paced
        features[0, 16] = 1.0 if features[0, 2] < 66.0 else 0.0  # is_slow_paced

        # Conference features (17-20)
        features[0, 17] = home_power
        features[0, 18] = away_power
        features[0, 19] = same_conf
        features[0, 20] = home_power + away_power  # total_power_conf_teams

        # Ranking features (21-24)
        features[0, 21] = home_rank_inv
        features[0, 22] = away_rank_inv
        features[0, 23] = abs(home_rank_inv - away_rank_inv)  # rank_diff
    ...
    def predict(self, game_data: Dict, market_total: Optional[float] = None) -> Dict:
        ...
        # Get prediction from trained Random Forest model
        ...
        if hasattr(self.model, 'estimators_'):
            ...
            confidence = float(1 / (1 + std_dev / 10))
        else:
            std_dev = 11.5
            confidence = 0.70
