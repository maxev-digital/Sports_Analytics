"""
NCAAB LightGBM Model for Totals Prediction
...
class NCAABLightGBMModel:
    """LightGBM model for NCAAB totals using KenPom methodology"""

    def __init__(self):
        """Initialize NCAAB LightGBM model"""
    ...
    def predict(self, game_data: Dict, market_total: Optional[float] = None) -> Dict:
        ...
        # LightGBM - faster and often more accurate
        expected_pace = np.sqrt(home_pace * away_pace)
        league_avg_eff = 100.0
        home_court_adv = 3.0

        # LightGBM learns feature interactions efficiently
        offense_dominance = (home_off + away_off) / 2 - league_avg_eff
        pace_boost = 1.0 + (offense_dominance / 1000.0)  # Slight boost for high-scoring teams

        home_expected_eff = (home_off - (away_def - league_avg_eff) + home_court_adv) * pace_boost
        away_expected_eff = (away_off - (home_def - league_avg_eff)) * pace_boost
        ...
        if is_live and current_score and quarter and time_remaining_str:
            ...
            try:
                ...
                if espn_stats:
                    ...
                    logger.info(f"LightGBM live projection: {analysis['explanation']}")
                else:
        ...
        # LightGBM confidence - often higher due to better leaf optimization
        pace_variance = abs(home_pace - away_pace) / expected_pace
        confidence = 0.75 - (pace_variance * 0.08)
        confidence = max(0.62, min(0.90, confidence))
        ...
        result = {
            ...
            'feature_importance': {
                'home_pace': 0.22,
                'away_pace': 0.20,
                'home_off_rating': 0.23,
                'away_off_rating': 0.21,
...
def get_ncaab_lightgbm_model():
    """Get or create singleton NCAAB LightGBM model instance"""
    global _model_instance
    if _model_instance is None:
        _model_instance = NCAABLightGBMModel()
