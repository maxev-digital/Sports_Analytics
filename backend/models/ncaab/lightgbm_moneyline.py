"""
LightGBM Model for NCAAB Moneyline Prediction
Fastest gradient boosting for college basketball win probability
"""
...
class LightGBMMoneylineModel:
    """LightGBM classifier for predicting NCAAB moneyline outcomes"""

    def __init__(self):
        model_dir = Path(__file__).parent.parent.parent / "ml" / "models"
        self.model = joblib.load(model_dir / "ncaab_lightgbm_moneyline_latest.joblib")
    ...
    def _extract_features(self, game_data: Dict) -> np.ndarray:
        ...
        features[0, 3:9] = [home.get('adj_off_eff', 105.0), away.get('adj_off_eff', 105.0),
                            home.get('adj_def_eff', 100.0), away.get('adj_def_eff', 100.0),
                            home.get('adj_tempo', 68.0), away.get('adj_tempo', 68.0)]
        features[0, 9:13] = [home.get('points_per_game', 72.0), away.get('points_per_game', 72.0),
                             home.get('points_allowed_per_game', 68.0), away.get('points_allowed_per_game', 68.0)]
        features[0, 13:19] = [home.get('fg_pct', 45.0), away.get('fg_pct', 45.0),
                              home.get('three_pt_pct', 34.0), away.get('three_pt_pct', 34.0),
                              home.get('ft_pct', 72.0), away.get('ft_pct', 72.0)]
        features[0, 19:25] = [home.get('rebound_margin', 0.0), away.get('rebound_margin', 0.0),
        ...
        features[0, 25:33] = [home.get('win_pct', 0.500), away.get('win_pct', 0.500),
    ...
    def predict(self, game_data: Dict, market_odds: Optional[Dict] = None) -> Dict:
        ...
        home_win_prob, away_win_prob, confidence = probabilities[1], probabilities[0], 0.77
...
def get_ncaab_lightgbm_moneyline_model():
