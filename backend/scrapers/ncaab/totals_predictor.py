...
class NCAABTotalsPredictor:
    ...
    def find_team(self, team_name):
        ...
        if original_name in self.team_stats:
            ...
            return self.team_stats[original_name], original_name
        
        # Step 2: Use mapping
        normalized = self.normalize_team_name(original_name)
        if normalized in self.team_stats:
            self.successful_lookups += 1
            return self.team_stats[normalized], normalized
        ...
        for kp_team in self.kenpom_teams:
            if kp_team.lower() == normalized_lower:
                self.successful_lookups += 1
                return self.team_stats[kp_team], kp_team
    ...
    def predict_game(self, home_team, away_team):
        ...
        home_stats, home_matched = self.find_team(home_team)
        away_stats, away_matched = self.find_team(away_team)
        
        if not home_stats or not away_stats:
            # Track which team(s) failed for debugging
            missing = []
            if not home_stats:
                missing.append(f"Home: {home_team}")
            if not away_stats:
                missing.append(f"Away: {away_team}")
            return None
    ...
    def get_lookup_stats(self):
        """Get team lookup statistics for debugging"""
        total = self.successful_lookups + self.failed_lookups
        if total == 0:
            return "No lookups performed yet"
        
        success_rate = (self.successful_lookups / total) * 100
        return {
...
def test_team_mapping():
    ...
    for test_name in test_cases:
        stats, matched_name = predictor.find_team(test_name)
        if stats:
            print(f"{test_name:<25} → {matched_name:<25} ✅")
            successful += 1
        else:
