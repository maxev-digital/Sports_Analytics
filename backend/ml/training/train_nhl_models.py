"""
NHL Model Training Script

Trains all 12 NHL models (Random Forest, XGBoost, LightGBM, Linear/Logistic Regression)
for Totals, Spreads, and Moneyline predictions.

Usage:
    python -m backend.ml.training.train_nhl_models --seasons 20222023 20232024
...
class NHLModelTrainer:
    """Train all NHL prediction models"""
    ...
    def train_totals_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NHL TOTALS models")
        logger.info("=" * 60)
        ...
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nhl_{model_name}_totals_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} totals model")

        # Save metadata
        joblib.dump(metadata, self.output_dir / "nhl_totals_metadata_latest.joblib")
        logger.info("  ✓ Saved totals metadata")
    ...
    def train_spreads_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NHL SPREADS models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nhl_{model_name}_spreads_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} spreads model")

        joblib.dump(metadata, self.output_dir / "nhl_spreads_metadata_latest.joblib")
        logger.info("  ✓ Saved spreads metadata")
    ...
    def train_moneyline_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NHL MONEYLINE models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nhl_{model_name}_moneyline_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} moneyline model")

        joblib.dump(metadata, self.output_dir / "nhl_moneyline_metadata_latest.joblib")
        logger.info("  ✓ Saved moneyline metadata")
    ...
    def _calculate_regression_metrics(self, y_true, y_pred, n_samples) -> Dict:
        ...
        # Additional betting-specific metrics
        errors = np.abs(y_true - y_pred)
        within_half_goal = np.mean(errors <= 0.5)
        within_one_goal = np.mean(errors <= 1.0)
        ...
        return {
            ...
            'rmse': float(rmse),
            'r2': float(r2),
            'within_half_goal': float(within_half_goal),
            'within_one_goal': float(within_one_goal),
...
def main():
    parser = argparse.ArgumentParser(description='Train NHL prediction models')
    parser.add_argument('--seasons', nargs='+', default=None,
                       help='Seasons to train on (e.g., 20222023 20232024)')
    ...
    # Load training data
    logger.info("Loading NHL training data...")
    df = load_nhl_training_data(seasons=args.seasons)
    ...
    # Train models
    trainer = NHLModelTrainer(output_dir=Path(args.output) if args.output else None)
    ...
    logger.info("=" * 60)
    logger.info("NHL MODEL TRAINING COMPLETE")
    logger.info("=" * 60)
