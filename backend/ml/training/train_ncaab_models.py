...
Trains all 12 NCAAB models (Random Forest, XGBoost, LightGBM, Linear/Logistic Regression)
for Totals, Spreads, and Moneyline predictions.

Usage:
    python -m backend.ml.training.train_ncaab_models --seasons 2023 2024 2025
...
class NCAABModelTrainer:
    ...
    def train_totals_models(self, df: pd.DataFrame) -> Dict:
        ...
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"ncaab_{model_name}_totals_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} totals model")

        # Save metadata
        joblib.dump(metadata, self.output_dir / "ncaab_totals_metadata_latest.joblib")
    ...
    def train_spreads_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NCAAB SPREADS models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"ncaab_{model_name}_spreads_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} spreads model")

        joblib.dump(metadata, self.output_dir / "ncaab_spreads_metadata_latest.joblib")
    ...
    def train_moneyline_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NCAAB MONEYLINE models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"ncaab_{model_name}_moneyline_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} moneyline model")

        joblib.dump(metadata, self.output_dir / "ncaab_moneyline_metadata_latest.joblib")
    ...
    def _calculate_regression_metrics(self, y_true, y_pred, n_samples) -> Dict:
        ...
        within_5_points = np.mean(errors <= 5.0)
        within_10_points = np.mean(errors <= 10.0)
        ...
        return {
            ...
            'rmse': float(rmse),
            'r2': float(r2),
            'within_5_points': float(within_5_points),
            'within_10_points': float(within_10_points),
...
def main():
    parser = argparse.ArgumentParser(description='Train NCAAB prediction models')
    ...
    # Load training data
    logger.info("Loading NCAAB training data...")
    df = load_ncaab_training_data(seasons=args.seasons)
    ...
    # Train models
    trainer = NCAABModelTrainer(output_dir=Path(args.output) if args.output else None)
    ...
    logger.info("=" * 60)
    logger.info("NCAAB MODEL TRAINING COMPLETE")
    logger.info("=" * 60)
