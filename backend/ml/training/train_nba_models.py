...
Trains all 12 NBA models (Random Forest, XGBoost, LightGBM, Linear/Logistic Regression)
for Totals, Spreads, and Moneyline predictions.

Usage:
    python -m backend.ml.training.train_nba_models --seasons 2022-23 2023-24 2024-25
...
class NBAModelTrainer:
    """Train all NBA prediction models"""
    ...
    def train_totals_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NBA TOTALS models")
        logger.info("=" * 60)
        ...
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nba_{model_name}_totals_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} totals model")

        # Save metadata
        joblib.dump(metadata, self.output_dir / "nba_totals_metadata_latest.joblib")
    ...
    def train_spreads_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NBA SPREADS models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nba_{model_name}_spreads_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} spreads model")

        joblib.dump(metadata, self.output_dir / "nba_spreads_metadata_latest.joblib")
    ...
    def train_moneyline_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NBA MONEYLINE models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nba_{model_name}_moneyline_latest.joblib")
            logger.info(f"  ✓ Saved {model_name} moneyline model")

        joblib.dump(metadata, self.output_dir / "nba_moneyline_metadata_latest.joblib")
...
def main():
    ...
    parser.add_argument('--seasons', nargs='+', default=None,
                       help='Seasons to train on (e.g., 2022-23 2023-24 2024-25)')
    ...
    # Train models
    trainer = NBAModelTrainer(output_dir=Path(args.output) if args.output else None)
