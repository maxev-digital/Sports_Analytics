...
Trains all 12 NFL models (Random Forest, XGBoost, LightGBM, Linear/Logistic Regression)
for Totals, Spreads, and Moneyline predictions.

Usage:
    python -m backend.ml.training.train_nfl_models --seasons 2022 2023 2024
...
class NFLModelTrainer:
    """Train all NFL prediction models"""
    ...
    def train_totals_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NFL TOTALS models")
        logger.info("=" * 60)
        ...
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nfl_{model_name}_totals_latest.joblib")
            logger.info(f"  [OK] Saved {model_name} totals model")

        # Save metadata
        joblib.dump(metadata, self.output_dir / "nfl_totals_metadata_latest.joblib")
    ...
    def train_spreads_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NFL SPREADS models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nfl_{model_name}_spreads_latest.joblib")
            logger.info(f"  [OK] Saved {model_name} spreads model")

        joblib.dump(metadata, self.output_dir / "nfl_spreads_metadata_latest.joblib")
    ...
    def train_moneyline_models(self, df: pd.DataFrame) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("Training NFL MONEYLINE models")
        logger.info("=" * 60)
        ...
        # Save
        for model_name, model in models.items():
            joblib.dump(model, self.output_dir / f"nfl_{model_name}_moneyline_latest.joblib")
            logger.info(f"  [OK] Saved {model_name} moneyline model")

        joblib.dump(metadata, self.output_dir / "nfl_moneyline_metadata_latest.joblib")
    ...
    def _calculate_regression_metrics(self, y_true, y_pred, n_samples) -> Dict:
        ...
        within_3_pts = np.mean(errors <= 3.0)  # Key number (field goal)
        within_7_pts = np.mean(errors <= 7.0)  # Key number (touchdown)
        within_10_pts = np.mean(errors <= 10.0)  # Key number (TD + FG)
        ...
        return {
            ...
            'rmse': float(rmse),
            'r2': float(r2),
            'within_3_pts': float(within_3_pts),
            'within_7_pts': float(within_7_pts),
            'within_10_pts': float(within_10_pts),
...
def main():
    ...
    df = load_nfl_training_data(seasons=args.seasons)
    ...
    # Train models
    trainer = NFLModelTrainer(output_dir=Path(args.output) if args.output else None)
    ...
    logger.info("=" * 60)
    logger.info("NFL MODEL TRAINING COMPLETE")
    logger.info("=" * 60)
