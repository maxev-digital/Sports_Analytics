...
def load_edge_lab_predictions(bet_type_filter: Optional[str] = None, model_filter: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            # Calculate calibrated model probability based on edge
            # Use sigmoid-like transformation: larger edges = higher probability
            # But cap at realistic ranges (55-70% for most plays)
            edge_pct = (abs(edge) / abs(market_value)) * 100 if market_value != 0 else 0
            
            # Base probability from edge percentage using calibrated sigmoid
            # Small edges (2-5%): 52-58% probability
            # Medium edges (5-10%): 58-65% probability  
            # Large edges (10%+): 65-70% probability
            import math
            base_prob = 0.50 + (1 / (1 + math.exp(-0.3 * (edge_pct - 5)))) * 0.20
            
            # Small boost for ensemble (multiple models agreeing = higher confidence)
            model_boost = 0.02 if model == 'ensemble' else 0.00
            
            # Calculate final model probability (realistic range: 52-72%)
            confidence = min(max(base_prob + model_boost, 0.52), 0.72)
