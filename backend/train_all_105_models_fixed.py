...
def train_sport_models(sport: str):
    ...
    for bet_type in bet_types:
        ...
        base_preds_tensor = torch.FloatTensor(base_preds)
        y_tensor = torch.FloatTensor(y_test)

        n_models = base_preds.shape[1]
        model_accs = torch.ones_like(base_preds_tensor) / n_models

        for epoch in range(50):
            optimizer.zero_grad()
            preds, _ = ensemble(base_preds_tensor, model_accs)
        ...
        with torch.no_grad():
            final_preds, _ = ensemble(base_preds_tensor, model_accs)
            predictions['ensemble'] = final_preds.numpy()

        # Calculate metrics
        for model_name, pred in predictions.items():
            if is_classifier:
                pred_binary = (pred > 0.5).astype(int) if model_name in ["pytorch", "ensemble"] else pred
