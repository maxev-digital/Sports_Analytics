...
def load_edge_lab_predictions(bet_type_filter: Optional[str] = None, model_filter: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        def should_include_game(row):
            ...
            # Daily sports: ONLY today's games (in CST timezone)
            if sport_code in ['NBA', 'NHL', 'NCAAB']:
                # Get today's date in CST
                today = now.date()
                # Convert game_date from UTC to CST for comparison
                game_date_cst = game_date.astimezone(pytz.timezone('America/Chicago'))
                game_day = game_date_cst.date()
                # Only include games happening today (CST)
        ...
        logger.info(f"Before date filtering: {len(df)} predictions, today is {now.date()}")
        df = df[df.apply(should_include_game, axis=1)]
        logger.info(f"After date filtering: {len(df)} predictions remaining")
        ...
        for _, row in df.iterrows():
            ...
            # Validate numeric values - skip predictions with NaN or Inf
            if any(pd.isna(v) or math.isnan(v) if isinstance(v, float) else False for v in [edge, predicted_value, market_value]):
                logger.warning(f"Skipping prediction with NaN values: {row['prediction_id']}")
                continue
            if any(math.isinf(v) if isinstance(v, float) else False for v in [edge, predicted_value, market_value]):
                logger.warning(f"Skipping prediction with Inf values: {row['prediction_id']}")
                continue
            # Skip if market_value is 0 (would cause division errors)
            if market_value == 0:
                logger.warning(f"Skipping prediction with market_value=0: {row['prediction_id']}")
                continue
            bet_type = row['bet_type']
            # Handle NaN model values (old format predictions)
            model_raw = row.get('model')
            if pd.isna(model_raw):
                model = 'ensemble'  # Default to ensemble for old predictions
            else:
                model = str(model_raw)
