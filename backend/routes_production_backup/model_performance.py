...
def safe_str(value):
    """Convert to string, returning None for NaN/None values"""
...
async def get_performance_overview(
    ...
    try:
        ...
        # Only include graded predictions (result is not null)
# 🛡️ SAFEGUARD #1: Filter out UNKNOWN results (games not yet graded)        merged_df = merged_df[merged_df["result"].isin(["WIN", "LOSS", "PUSH"])]                # 🛡️ SAFEGUARD #2: Exclude NONE confidence (low-edge predictions not placed as bets)        merged_df = merged_df[merged_df["confidence"].isin(["HIGH", "MEDIUM", "LOW"])]

        # 🛡️ SAFEGUARD #1: Filter out UNKNOWN results (games not yet graded)
        merged_df = merged_df[merged_df["result"].isin(["WIN", "LOSS", "PUSH"])]

        # 🛡️ SAFEGUARD #2: Exclude NONE confidence (low-edge predictions not placed as bets)
        merged_df = merged_df[merged_df["confidence"].isin(["HIGH", "MEDIUM", "LOW"])]
        merged_df = merged_df[merged_df['result'].notna()]
        ...
        # 🛡️ SAFEGUARD #3: ROI excludes pushes from denominator
        graded_bets = wins + losses
        roi = (total_profit_loss / graded_bets) * 100 if graded_bets > 0 else 0

        # Average edge
        avg_edge = merged_df.get('edge_accuracy', pd.Series([0])).abs().mean()
...
async def get_performance_history(
    ...
    try:
        ...
        # Only include graded predictions
# 🛡️ SAFEGUARD #1: Filter out UNKNOWN results (games not yet graded)        merged_df = merged_df[merged_df["result"].isin(["WIN", "LOSS", "PUSH"])]                # 🛡️ SAFEGUARD #2: Exclude NONE confidence (low-edge predictions not placed as bets)        merged_df = merged_df[merged_df["confidence"].isin(["HIGH", "MEDIUM", "LOW"])]
        merged_df = merged_df[merged_df['result'].notna()]
        ...
        # Create date column from game_date_dt for grouping
...
async def get_individual_predictions(
    ...
    try:
        ...
        # Only show graded predictions (result is not null)
        predictions_df = predictions_df[pd.notna(predictions_df['result'])]
        ...
        for idx, row in predictions_df.iterrows():
            try:
                pred = {
                    ...
                    'bet_placed': safe_str(row.get('bet_placed')),
