...
from pipeline.ingestion.mlb_park_factors import get_park_factor, apply_park_factor_to_prob
...
_MULTIBOOK_PROB_DIVERGENCE = 0.03      # 3% off consensus = flaggable
...
def _fetch_espn_starters() -> dict:
    """Return {team_name_lower: pitcher_name} from ESPN MLB scoreboard probable pitchers."""
    try:
            ...
            for comp in event.get("competitions", []):
                for competitor in comp.get("competitors", []):
                    ...
                    for p in competitor.get("probables", []):
                        ...
                        if name and team:
                            # Statcast stores names as "Last, First" — convert from ESPN's "First Last"
                            parts = name.rsplit(" ", 1)
                            if len(parts) == 2:
                                name = f"{parts[1]}, {parts[0]}"
                            starters[team.lower()] = name
                            break
        return starters
    except Exception as exc:
        logger.warning("[mlb_predictor] ESPN starters fetch failed: %s", exc)
        return {}
...
def rule_based_mlb_edges(min_edge: float = 3.0) -> list[dict]:
    ...
    # Enrich game objects with probable starters from ESPN (OddsAPI doesn't provide SP names)
    espn_starters = _fetch_espn_starters()
    ...
    for game in games:
        ...
        # Skip games that have already started — in-game odds are unreliable for pre-game picks
        try:
            raw_ct = game.get("commence_time", "")
            if raw_ct:
                game_start = datetime.fromisoformat(raw_ct)
                if game_start.tzinfo is None:
                    game_start = game_start.replace(tzinfo=_tz.utc)
                if game_start <= datetime.now(_tz.utc):
                    logger.debug("[mlb_predictor] Skipping in-progress game: %s @ %s", away_team, home_team)
                    continue
        except Exception:
            pass

        # Fetch SP stats once per game — shared across detectors
        home_sp_name = (
            game.get("home_starter")
            or game.get("home_sp")
            or espn_starters.get(home_team.lower())
        )
        away_sp_name = (
            game.get("away_starter")
            or game.get("away_sp")
            or espn_starters.get(away_team.lower())
        )
        ...
        try:
            if home_sp_name and away_sp_name:
                home_sp = home_sp_game
                away_sp = away_sp_game
                    ...
                    if total_data:
                        ...
                        if over_odds and under_odds and total_point:
                            if combined_gap > _PITCHING_LUCK_OVER_THRESHOLD:
                                ...
                                # Park factor: hitter-friendly parks amplify the over signal
                                park_adj = (get_park_factor(home_team) - 1.0) * 0.30
                                our_prob = min(fair_over + luck_boost + park_adj, 0.87)
                                ...
                                if edge_pct >= min_edge:
                                    picks.append(_build_pick(
                                        ...
                                        features={
                                            ...
                                            "park_factor": round(get_park_factor(home_team), 3),
                                        },
                                    ))
                                ...
                                # Park factor: pitcher-friendly parks amplify the under signal
                                park_adj = -(get_park_factor(home_team) - 1.0) * 0.30
                                our_prob = min(fair_under + luck_boost + park_adj, 0.87)
        ...
        try:
            for market_key, pick_type in [("h2h", "ml"), ("totals", "total")]:
                ...
                # Collect per-book implied probabilities: (book_key, primary_ip, primary_odds, other_odds)
                # primary = home for h2h, over for totals
                book_probs: list[tuple[str, float, int, int]] = []
                    ...
                    if market_key == "h2h":
                        ...
                        if home_out and away_out:
                            ...
                            if ho and ao:
                                ...
                                book_probs.append((bk_key, ip, int(ho), int(ao)))
                        ...
                        if over_out and under_out:
                            ...
                            if oo and uo:
                                ...
                                book_probs.append((bk_key, ip, int(oo), int(uo)))
                ...
                all_probs = [p for _, p, _, _ in book_probs]
                ...
                for bk_key, bk_prob, bk_primary_odds, bk_other_odds in book_probs:
                    ...
                    if abs(divergence) >= _MULTIBOOK_PROB_DIVERGENCE and abs(divergence) < 0.15:
                        if divergence < 0:
                            # Primary side is cheap at this book — value on over/home
                            our_prob = consensus
                            edge_pct = (consensus - bk_prob) * 100
                            pick_side = "over" if market_key == "totals" else "home"
                            market_odds = bk_primary_odds
                            market_implied = bk_prob
                        else:
                            # Primary side is expensive — value on under/away at this book
                            other_ip = _american_to_implied(bk_other_odds)
                            fair_other_prob = 1.0 - consensus
                            edge_pct = (fair_other_prob - other_ip) * 100
                            our_prob = fair_other_prob
                            pick_side = "under" if market_key == "totals" else "away"
                            market_odds = bk_other_odds
                            market_implied = other_ip
        ...
        # ── Detector 4: K-Rate Totals (high strikeout matchup → lean under) ──────
        try:
            home_xwoba = home_sp_game.get("est_woba")
            away_xwoba = away_sp_game.get("est_woba")
            if home_xwoba is not None and away_xwoba is not None:
                combined_xwoba = float(home_xwoba) + float(away_xwoba)
                if combined_xwoba < 0.590:
                    total_data = extract_consensus_line(game, "totals")
                    if total_data:
                        over_odds = total_data.get("over_odds")
                        under_odds = total_data.get("under_odds")
                        ...
                        if over_odds and under_odds and total_point:
                            fair_under = _fair_prob_from_vig_removal(under_odds, over_odds)
                            xwoba_gap = 0.590 - combined_xwoba
                            k_boost = min(xwoba_gap * 0.35, 0.07)
                            park_adj = -(get_park_factor(home_team) - 1.0) * 0.20
                            our_prob = min(fair_under + k_boost + park_adj, 0.82)
                            edge_pct = (our_prob - fair_under) * 100
                            if edge_pct >= min_edge:
                                picks.append(_build_pick(
                                    game=game,
                                    ...
                                    market_odds=int(under_odds),
                                    market_implied=fair_under,
                                    detector="rule_krate_xwoba",
                                    features={
                                        "home_sp": home_sp_name,
                                        "away_sp": away_sp_name,
                                        "home_est_woba_allowed": round(float(home_xwoba), 4),
                                        "away_est_woba_allowed": round(float(away_xwoba), 4),
                                        "combined_est_woba": round(combined_xwoba, 4),
                                        "xwoba_gap_below_avg": round(xwoba_gap, 4),
                                        "park_factor": round(get_park_factor(home_team), 3),
                                        "total_line": total_point,
                                    },
                                ))
    ...
    for pick in picks:
        key = f"{pick.get('game_id', '')}_{pick['pick_type']}_{pick['pick_side']}"
        if key not in seen_per_side or pick["edge_pct"] > seen_per_side[key]["edge_pct"]:
            seen_per_side[key] = pick

    # Deduplicate step 2: opposite sides on same game + market are mutually exclusive
    # (home/away, over/under, home_cover/away_cover) — keep only highest edge
    by_market: dict[str, list] = {}
    for pick in seen_per_side.values():
        market_key = f"{pick.get('game_id', '')}_{pick['pick_type']}"
        by_market.setdefault(market_key, []).append(pick)

    final_picks: list[dict] = []
    for market_picks in by_market.values():
        if len(market_picks) == 1:
            final_picks.extend(market_picks)
        else:
            final_picks.append(max(market_picks, key=lambda p: p["edge_pct"]))

    result = sorted(final_picks, key=lambda p: p["edge_pct"], reverse=True)
    logger.info(
        ...
        for (pitcher, k_line), book_list in pitcher_line_books.items():
            ...
            for bk_key, bk_prob, over_odds, under_odds in book_list:
                ...
                if (
                    ...
                    if edge_pct >= min_edge:
                        # Enrich pick with pitcher quality context from Statcast
                        pitcher_ctx = _safe_pitcher_stats(pitcher, "k_prop")
                        xwoba_allowed = pitcher_ctx.get("est_woba") if pitcher_ctx else None
                        era_gap = pitcher_ctx.get("era_luck_gap") if pitcher_ctx else None
                        pitcher_tier = None
                        if xwoba_allowed is not None:
                            x = float(xwoba_allowed)
                            if x < 0.285: pitcher_tier = "elite"
                            elif x < 0.305: pitcher_tier = "above_avg"
                            elif x < 0.325: pitcher_tier = "avg"
                            else: pitcher_tier = "below_avg"
                        ...
                        picks.append(_build_pick(
                            ...
                            features={
                                ...
                                "home_team": home_team,
                                "away_team": away_team,
                                "pitcher_est_woba_allowed": round(float(xwoba_allowed), 4) if xwoba_allowed is not None else None,
                                "pitcher_era_luck_gap": round(float(era_gap), 3) if era_gap is not None else None,
                                "pitcher_quality_tier": pitcher_tier,
                            },
                        ))
