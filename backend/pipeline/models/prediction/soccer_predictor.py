...
def _fetch_team_goal_form(team_name: str, league: str) -> Optional[dict]:
    """
    Fetch a team's recent scoring form from ESPN soccer.
    Returns dict with avg_goals_scored, avg_goals_allowed, or None on failure.
    """
    try:
        import httpx
        league_slug = _ESPN_SOCCER_LEAGUES.get(league, "usa.1")
            ...
            for comp in event.get("competitions", []):
                ...
                for i, c in enumerate(competitors):
                    if team_name.lower() in c.get("team", {}).get("displayName", "").lower():
                        opp = competitors[1 - i] if len(competitors) > 1 else None
                        try:
                            scored = int(c.get("score", 0))
                            allowed = int(opp.get("score", 0)) if opp else 0
                            goals_scored.append(scored)
                            goals_allowed.append(allowed)
                        except (TypeError, ValueError):
                            pass
        if len(goals_scored) < 2:
            return None
        return {
            "avg_goals_scored": round(sum(goals_scored) / len(goals_scored), 2),
            "avg_goals_allowed": round(sum(goals_allowed) / len(goals_allowed), 2),
            "games_sampled": len(goals_scored),
        }
    except Exception:
        return None
    ...
    for league, sport_key in SPORT_KEYS.items():
        ...
        for game in windowed:
            ...
            _detect_totals(picks, sport_label, game, game_time_cst, bookmakers,
                           home_team, away_team, min_edge, league)
                ...
                for bk_key, bk_prob, bk_odds in side_probs:
                    ...
                    if (divergence < 0
                        ...
                        if edge_pct >= min_edge:
                            home_form = _fetch_team_goal_form(home_team, league)
                            away_form = _fetch_team_goal_form(away_team, league)
                            combined_avg = None
                            if home_form is not None or away_form is not None:
                                combined_avg = round(
                                    (home_form["avg_goals_scored"] if home_form else 0) +
                                    (away_form["avg_goals_scored"] if away_form else 0), 2
                                )
                            picks.append(_build_pick(
                                ...
                                features={
                                    ...
                                    "home_avg_goals_scored": home_form["avg_goals_scored"] if home_form else None,
                                    "away_avg_goals_scored": away_form["avg_goals_scored"] if away_form else None,
