...
from pipeline.ingestion.nfl_ingestion import run_nfl_ingestion
from pipeline.narrative_pipeline import run_narrative_generation
from pipeline.ingestion.team_ratings_ingestion import run_ratings_ingestion
from pipeline.models.prediction.nfl_detector import rule_based_nfl_edges
from pipeline.models.prediction.ncaaf_detector import rule_based_ncaaf_edges
...
def run_predictions(use_ml: bool = True) -> list[dict]:
    ...
    # NFL (Sep-Feb window; returns [] outside season)
    try:
        nfl_picks = rule_based_nfl_edges()
        all_picks.extend(nfl_picks)
        logger.info(f"NFL: {len(nfl_picks)} picks generated")
    except Exception as e:
        logger.error(f"NFL prediction failed: {e}")

    # NCAAF (Aug-Jan window; returns [] outside season)
    try:
        ncaaf_picks = rule_based_ncaaf_edges()
        all_picks.extend(ncaaf_picks)
        logger.info(f"NCAAF: {len(ncaaf_picks)} picks generated")
    except Exception as e:
        logger.error(f"NCAAF prediction failed: {e}")
...
def save_picks(picks: list[dict], skip_indices: list[int] = None) -> int:
    ...
    # One pick per game_id: keep highest-edge pick, drop both-sides and dupes
    seen_games = {}
    single_picks = []
    for pick in picks:
        ...
        if not gid:
            ...
            continue
        if gid not in seen_games or pick.get('edge_pct', 0) > seen_games[gid].get('edge_pct', 0):
            seen_games[gid] = pick
    single_picks += list(seen_games.values())
    if len(single_picks) < len(picks):
        logger.info('[save_picks] Dropped %d both-sides/dupe picks before save', len(picks) - len(single_picks))
    picks = single_picks
    saved = 0
...
def run_daily_pipeline(use_ml: bool = False) -> dict:
    ...
    # Phase 4 — Game Script Narratives (Sonnet agent, fails-open)
    try:
        narratives_generated = run_narrative_generation()
        logger.info(f"Narrative pipeline: {narratives_generated} scripts generated")
    except Exception as e:
        logger.warning(f"Narrative pipeline failed (non-critical): {e}")

    # Phase 1 — Ingest
    # run_nfl_ingestion covers both NFL + NCAAF odds; returns [] outside season windows
    ingestion_reports = [run_mlb_ingestion(), run_wnba_ingestion(), run_nfl_ingestion()]
    # Team ratings: NFL power ratings (PFR) + NCAAF SP+ — non-critical, run separately
    try:
        run_ratings_ingestion()
        logger.info("Team ratings ingestion complete")
    except Exception as e:
        logger.warning(f"Team ratings ingestion failed (non-critical): {e}")
