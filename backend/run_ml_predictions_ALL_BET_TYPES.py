...
def load_sport_models(sport, bet_type="totals"):
    ...
    for name in ['xgboost', 'lightgbm', 'random_forest', 'linear', 'catboost']:
        ...
        if model_path.exists():
            if name == 'catboost':
                # CatBoost models must be loaded with .load_model()
...
def extract_market_line(game, bet_type):
    """Extract the appropriate market line based on bet type"""
    ...
    if bet_type == 'totals':
        # Get all totals from bookmakers
        totals = [odd['total'] for odd in odds_list if odd.get('total')]
        if totals:
            result['market_value'] = statistics.median(totals)
            over_odds = [odd['over_price'] for odd in odds_list if odd.get('over_price')]
            under_odds = [odd['under_price'] for odd in odds_list if odd.get('under_price')]
            if over_odds:
                result['over_odds'] = int(statistics.median(over_odds))
            if under_odds:
                result['under_odds'] = int(statistics.median(under_odds))
    
    elif bet_type == 'spreads':
        home_team_state = game_state.get('home_team', {})
        spread = home_team_state.get('spread')
        if spread is not None:
            result['market_value'] = spread
            spread_odds = [odd['home_spread_price'] for odd in odds_list if odd.get('home_spread_price')]
            if spread_odds:
                result['spread_odds'] = int(statistics.median(spread_odds))
    
    elif bet_type == 'moneyline':
        home_team_state = game_state.get('home_team', {})
        away_team_state = game_state.get('away_team', {})
        home_ml = home_team_state.get('money_line')
        away_ml = away_team_state.get('money_line')
        if home_ml is not None:
            result['home_ml'] = home_ml
            result['market_value'] = home_ml
...
def calculate_edge_and_recommendation(predicted, market_value, bet_type, market_data, home_team=None, away_team=None):
    """Calculate edge, recommendation, probability and kelly based on bet type"""
    
    probability = 0.5  # Default
    kelly_pct = 0.0    # Default
    
    if bet_type == 'totals':
        # Predicted is total points, market is total line
        edge = ((predicted - market_value) / market_value) * 100
        
        # Calculate probability based on how far over/under
        diff = predicted - market_value
        probability = 0.5 + min(0.45, (diff / market_value) * 2)
        ...
        # Predicted is margin (home - away), Market is home spread
        # Edge stays in POINTS for spreads
        edge = predicted - market_value
        
        # Probability based on point differential
        probability = 0.5 + min(0.4, abs(edge) / 20)
        probability = max(0.55, probability) if abs(edge) > 3 else 0.5
        
        if edge > 3:
            recommendation = home_team if (market_value < 0 and home_team) else (away_team if away_team else "AWAY")
            confidence = "HIGH" if abs(edge) > 7 else "MEDIUM"
        elif edge < -3:
            recommendation = away_team if (market_value < 0 and away_team) else (home_team if home_team else "HOME")
            confidence = "HIGH" if abs(edge) > 7 else "MEDIUM"
        else:
            recommendation = "NO BET"
            confidence = "LOW"
        ...
        # Predicted is home win probability
        home_ml = market_data.get('home_ml')
        
        if home_ml:
            market_prob = american_odds_to_probability(home_ml)
            edge = (predicted - market_prob) * 100
            probability = predicted
            
            if edge > 5:
                recommendation = home_team if home_team else "HOME"
                confidence = "HIGH" if edge > 10 else "MEDIUM"
            elif edge < -5:
                recommendation = away_team if away_team else "AWAY"
                confidence = "HIGH" if edge < -10 else "MEDIUM"
                probability = 1 - predicted
            else:
                recommendation = "NO BET"
                confidence = "LOW"
        else:
    ...
    # Kelly criterion
    if abs(edge) > 0 and probability > 0.5:
        decimal_odds = 1.91  # -110 odds
        b = decimal_odds - 1
        kelly_pct = (b * probability - (1 - probability)) / b
        kelly_pct = max(0, min(0.25, kelly_pct))
    
    return edge, recommendation, confidence, probability, kelly_pct
...
def extract_features_with_real_stats(game, sport, feature_engineer_class, teams_cache, bet_type="totals"):
    ...
    try:
        ...
        # Extract features using sport-specific engineer based on bet_type
        if bet_type == "totals":
            features = feature_engineer_class.get_totals_features(game_data)
        elif bet_type == "spreads":
            features = feature_engineer_class.get_spreads_features(game_data)
        elif bet_type == "moneyline":
            features = feature_engineer_class.get_moneyline_features(game_data)
        else:
            features = feature_engineer_class.get_totals_features(game_data)
        return features
...
def predict_for_sport(sport, games, models, config, teams_cache, bet_type="totals"):
    ...
    for game in sport_games:
        try:
            ...
            features = extract_features_with_real_stats(game, sport, feature_engineer, teams_cache, bet_type)
            if features is None:
                continue
            ...
            for name, model in models.items():
                # For classification models (moneyline), use predict_proba
                if bet_type == 'moneyline' and hasattr(model, 'predict_proba'):
                    # Get probability of home team winning (class 1)
                    pred = model.predict_proba(features)[0][1]
                else:
                    # Regression models (totals, spreads)
            ...
            # Apply bet-type-specific valid ranges
            if bet_type == 'totals':
                min_val, max_val = config['valid_range']
                ensemble = max(min_val, min(max_val, ensemble))
            elif bet_type == 'spreads':
                # Spreads can be negative, clamp to reasonable range
                ensemble = max(-50, min(50, ensemble))
            elif bet_type == 'moneyline':
                # Moneyline is probability 0-1
                ensemble = max(0.01, min(0.99, ensemble))

            # Extract real market lines from game API data
            market_data = extract_market_line(game, bet_type)
            market_value = market_data.get('market_value')
            
            # Fallback to default if no market data available
            if market_value is None:
                market_value = config['default_total']
            
            game_state = game.get('state', {})
            # Calculate edge and recommendation based on bet type
            edge, recommendation, confidence, probability, kelly_pct = calculate_edge_and_recommendation(
                ensemble, market_value, bet_type, market_data, 
            ...
            if commence_time_utc:
                ...
                game_date = ''
                game_time = ''
            pred_id = f"{sport.lower()}_{bet_type}_{game_date}_{home_team.replace(' ', '_')}"
            ...
            predictions.append({
                ...
                'over_probability': round(probability, 4),
                'edge': round(edge, 2),
...
def main():
    ...
    for sport, config in SPORT_CONFIG.items():
        ...
        for bet_type in bet_types:
            ...
            sport_predictions = predict_for_sport(sport, games, models, config, teams_cache, bet_type)
